...
import { 
  ...
  Heart, Check, AlertCircle, User, HelpCircle, Leaf, Home
} from 'lucide-react';
...
const MOCK_POSTS = [
  { id: '1', user: 'Joël Mikam', text: 'Just won a huge P2P pool on the Madrid game! MJ NEXUS is truly transparent. 🚀', likes: 0, comments: 0, timestamp: '2 hours ago', image: null },
  { id: '2', user: 'Sarah K.', text: 'Looking for a betting partner for the NBA finals. DM me!', likes: 0, comments: 0, timestamp: '5 hours ago', image: null },
];
...
const MOCK_CHALLENGES = [
  { id: 'c1', category: 'Betting', title: 'The Winning Streak', desc: 'Win 3 P2P pools in a row', reward: 500, progress: 0, total: 3, icon: Trophy, color: 'emerald' },
  { id: 'c2', category: 'Social', title: 'Social Butterfly', desc: 'Get 50 likes on your posts', reward: 200, progress: 0, total: 50, icon: MessageSquare, color: 'blue' },
  ...
  { id: 'c4', category: 'Home Services', title: 'Safe Home Hero', desc: 'Book 5 Servisécur interventions', reward: 750, progress: 0, total: 5, icon: Shield, color: 'indigo' },
  { id: 'c5', category: 'Betting', title: 'High Roller', desc: `Participate in a pool over ${MOCK_COUNTRIES.find(c => c.code === 'US')?.symbol}1000`, reward: 300, progress: 0, total: 1, icon: Target, color: 'rose' },
  { id: 'c6', category: 'Marketplace', title: 'Bulk Buyer', desc: 'Purchase 10 items in a single B2B order', reward: 400, progress: 0, total: 10, icon: Store, color: 'cyan' },
];
...
const SecurityGate = ({ children, requiredLevel, currentLevel, isSessionVerified, onVerify }: any) => {
  ...
  // DG (L6) bypasses the gate if they are already logged in as DG
  if (currentLevel >= 6 && requiredLevel <= 6) return children;
    ...
    <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-100 shadow-xl max-w-md mx-auto mt-20">
      ...
      <div className="w-full space-y-4">
        ...
        <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
          <div className="text-[9px] font-bold text-slate-400 uppercase mb-1">Security Hint</div>
          <div className="text-[10px] text-slate-500 italic">
            {requiredLevel === 3 && "Supervisor Key: nexus-sup-2025"}
            {requiredLevel === 4 && "Country Key: nexus-country-2025"}
            {requiredLevel === 5 && "Region Key: nexus-region-2025"}
            {requiredLevel === 6 && "DG Key: nexus-dg-2025"}
          </div>
        </div>
...
export default function App() {
  ...
  const initiateBooking = (service: string) => {
    const message = `Bonjour MJ NEXUS, je souhaite réserver une intervention pour le service : ${service}.`;
    window.open(`https://wa.me/237699932926?text=${encodeURIComponent(message)}`, '_blank');
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setUserRole(data.user.role === 'admin' ? 6 : 1);
  ...
  const handlePurchase = (product: any) => {
    const message = `Bonjour MJ NEXUS, je souhaite acheter le produit suivant : ${product.name} (${product.price} USD).`;
    window.open(`https://wa.me/237699932926?text=${encodeURIComponent(message)}`, '_blank');
    
    ReactGA.event({
      category: 'Marketplace',
      action: 'WhatsApp Purchase',
      label: product.name,
  ...
  const handleBookingSubmit = () => {
    const message = `Bonjour MJ NEXUS, je souhaite réserver une intervention pour le service : ${bookingData.service}.`;
    window.open(`https://wa.me/237699932926?text=${encodeURIComponent(message)}`, '_blank');
  ...
  const handleWalletAction = async () => {
    const message = `Bonjour MJ NEXUS, je souhaite effectuer une opération sur mon portefeuille (Dépôt/Retrait) d'un montant de ${momoAmount} USD.`;
    window.open(`https://wa.me/237699932926?text=${encodeURIComponent(message)}`, '_blank');
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="space-y-20 -mt-8">
            ...
            <section className="relative h-[600px] flex items-center justify-center overflow-hidden rounded-3xl">
              <div className="absolute inset-0 bg-slate-900">
                <div className="absolute inset-0 opacity-20 bg-[url('https://picsum.photos/seed/douala/1920/1080')] bg-cover bg-center mix-blend-overlay" />
                <div className="absolute inset-0 bg-gradient-to-r from-slate-900 via-slate-900/80 to-transparent" />
              </div>
              
              <div className="relative z-10 max-w-4xl px-8 text-center md:text-left md:mr-auto">
                <motion.div
                ...
                >
                  <h1 className="text-5xl md:text-7xl font-black text-white leading-tight mb-6">
                    MJ NEXUS : <br />
                    <span className="text-emerald-500">Votre Partenaire Business</span> <br />
                    à Douala
                  </h1>
                  <p className="text-xl text-slate-300 mb-10 max-w-2xl leading-relaxed">
                    L'écosystème unifié qui révolutionne le commerce, les services et la technologie au Cameroun. 
                    Qualité, rapidité et innovation à votre portée.
                  </p>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button 
                      ...
                      className="px-8 py-4 bg-emerald-500 text-white rounded-2xl font-bold text-lg hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2"
                    >
              ...
              <motion.div 
                ...
                className="hidden lg:block absolute right-12 top-1/2 -translate-y-1/2 w-[450px] h-[450px] bg-emerald-500/10 rounded-full border border-emerald-500/20 backdrop-blur-3xl"
              >
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-64 h-64 bg-slate-900 rounded-3xl shadow-2xl flex items-center justify-center border border-slate-800 rotate-12">
                    <Logo size="lg" />
                  </div>
                </div>
              </motion.div>
            ...
            <section id="services-grid" className="py-12">
              <div className="text-center mb-16">
                <h2 className="text-4xl font-black text-slate-900 mb-4">Nos Services d'Excellence</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  {
                    title: "High-Tech",
                    desc: "Vente de gadgets de pointe : Oraimo, Hoco, Calus. Le meilleur de la technologie mondiale à Douala.",
                    icon: Smartphone,
                    color: "blue",
                    tab: "marketplace"
                  },
                  {
                    ...
                    desc: "Kits solaires complets et solutions énergétiques durables pour particuliers et entreprises.",
                    icon: Zap,
                    color: "amber",
                    tab: "marketplace"
                  },
                  {
                    title: "Écologie",
                    desc: "Production de bio-charbon et pavés écologiques. Construisons ensemble un avenir vert.",
                    icon: Leaf,
                    color: "emerald",
                    tab: "marketplace"
                  },
                  {
                    title: "Logistique",
                    desc: "Service de livraison ultra-rapide dans tout Douala. Vos colis en sécurité, partout, tout le temps.",
                    icon: Truck,
                    color: "indigo",
                    tab: "logistics"
                  }
                  ...
                  <motion.div
                    key={i}
                    whileHover={{ y: -10 }}
                    className="bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col items-center text-center group cursor-pointer"
                    onClick={() => setActiveTab(service.tab as any)}
                  >
                    <div className={`w-16 h-16 rounded-2xl mb-6 flex items-center justify-center transition-all group-hover:scale-110 ${
                    ...
                    <div className="mt-auto text-emerald-500 font-bold text-xs uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">
            ...
            {/* Contact Section */}
            <section className="bg-slate-900 rounded-[3rem] p-12 md:p-20 overflow-hidden relative">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
                <div>
                  <h2 className="text-4xl md:text-5xl font-black text-white mb-6">Contactez MJ NEXUS</h2>
                  <p className="text-slate-400 text-lg mb-10 leading-relaxed">
                    Une question ? Un projet ? Notre équipe est à votre écoute pour vous accompagner dans votre croissance.
                  </p>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-4 text-white">
                      ...
                      <div>
                        <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Localisation</div>
                        <div className="font-bold">Akwa, Douala, Cameroun</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 text-white">
                      <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center border border-white/10">
                      ...
                      <div>
                        <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">WhatsApp</div>
                        <div className="font-bold">+237 699 932 926</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-12">
                    <button 
                      onClick={() => window.open('https://wa.me/237699932926', '_blank')}
                      className="px-10 py-5 bg-emerald-500 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-2xl shadow-emerald-500/40 flex items-center gap-3"
                    >
                ...
                <div className="bg-white rounded-3xl p-8 shadow-2xl">
                  <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); alert('Message envoyé ! Nous vous répondrons sous peu.'); }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Nom Complet</label>
                        <input type="text" placeholder="John Doe" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500" required />
                      </div>
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Email</label>
                        <input type="email" placeholder="john@example.com" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500" required />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Sujet</label>
                      <select className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500">
                    ...
                    <div>
                      ...
                      <textarea placeholder="Comment pouvons-nous vous aider ?" className="w-full h-32 bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm focus:outline-none focus:border-emerald-500 resize-none" required></textarea>
                    </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <StatCard label="Total Balance" value={formatPrice(balance)} trend={0} icon={Wallet} />
              <StatCard label="Active Bets" value="0" trend={0} icon={TrendingUp} />
              <StatCard label="P2P Pools" value="0" trend={0} icon={Users} />
              <StatCard label="Loyalty Points" value="0" trend={0} icon={Star} />
            </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="data-card p-6 flex flex-col items-center justify-center text-center">
                            ...
                            <h5 className="text-xl font-black text-slate-900">0</h5>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      ...
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                        ...
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <button 
                            onClick={() => {
                              if (referralInfo?.referralCode) {
                                const text = `Rejoins-moi sur MJ NEXUS ! Utilise mon code ${referralInfo.referralCode} pour obtenir un bonus de bienvenue. ${window.location.origin}`;
                  ...
                  <div className="bg-white/5 rounded-3xl p-6 border border-white/10 flex flex-col justify-between">
                    ...
                    <button 
                      ...
                      className="w-full mt-6 py-4 bg-emerald-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20"
                    >
                      Invite Friends Now
                    </button>
                  </div>
                        ...
                        {MOCK_CRYPTO.map(coin => (
                          <tr key={coin.id} className="hover:bg-slate-50 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                              <button 
                                onClick={() => {
                                  alert(`Ouverture de l'interface de trading pour ${coin.symbol}...`);
                                }}
                                className="px-4 py-1.5 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-emerald-600 transition-all"
                              >
                                Trade
                              </button>
                            </td>
      ...
      case 'help':
        return (
          <Suspense fallback={<div className="p-12 text-center text-slate-400 animate-pulse">Chargement de l'aide...</div>}>
            <HelpPage />
          </Suspense>
        );
      case 'privacy':
        return (
          ...
          <Suspense fallback={<div className="p-12 text-center text-slate-400 animate-pulse">Chargement...</div>}>
            <PrivacyCenter />
          </Suspense>
          </div>
        );
            ...
            {!myShop ? (
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <StatCard label="Total Sales" value={formatPrice(myShop.sales)} trend={15.4} icon={TrendingUp} />
                  <StatCard label="Net Profit" value={formatPrice(myShop.sales * 0.88)} trend={12.2} icon={Coins} />
                  <StatCard label="Global Reach" value="12 Countries" trend={8.5} icon={Globe} />
                  <StatCard label="Customer Satisfaction" value="4.9/5" trend={0.5} icon={Heart} />
                </div>
      ...
      case 'ecosystem':
        return (
          <div className="space-y-8">
            ...
            {/* Douala Logistics & Storage Section */}
            <div className="data-card border-emerald-500/30 bg-emerald-50/30">
              <div className="flex flex-col lg:flex-row gap-8 items-center">
                <div className="flex-1 space-y-6">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500 text-white rounded-full text-[10px] font-black uppercase tracking-widest">
                    <Truck size={12} /> Service de Livraison Douala
                  </div>
                  <h3 className="text-3xl font-black text-slate-900 leading-tight">
                    MJ Logistics: <span className="text-emerald-600">Vendez à Douala</span> sans frontières.
                  </h3>
                  <p className="text-slate-600 text-sm leading-relaxed">
                    Nous offrons une solution complète de stockage et de livraison pour les e-commerçants du monde entier souhaitant conquérir le marché de Douala, Cameroun.
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-white rounded-2xl border border-emerald-100 shadow-sm">
                      <div className="text-[10px] font-black text-emerald-600 uppercase mb-1">Clients Résidents (+237)</div>
                      <div className="text-2xl font-black text-slate-900">1 500 F <span className="text-xs font-medium text-slate-400">/ livraison</span></div>
                      <p className="text-[10px] text-slate-500 mt-2">Forfait unique dans toute la ville de Douala.</p>
                    </div>
                    <div className="p-4 bg-slate-900 rounded-2xl text-white shadow-xl">
                      <div className="text-[10px] font-black text-emerald-400 uppercase mb-1">Entreprises Internationales</div>
                      <div className="text-2xl font-black">2 000 F <span className="text-xs font-medium text-slate-400">/ livraison</span></div>
                      <div className="mt-2 pt-2 border-t border-white/10">
                        <div className="text-[10px] font-bold text-slate-400">Stockage Mensuel</div>
                        <div className="text-sm font-bold text-emerald-400">20 000 F - 25 000 F</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <a 
                      href="https://wa.me/237699932926" 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 px-6 py-3 bg-[#25D366] text-white rounded-xl font-bold text-sm hover:scale-105 transition-all shadow-lg"
                    >
                      <MessageCircle size={18} /> WhatsApp
                    </a>
                    <a 
                      href="tel:+237672175723" 
                      className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl font-bold text-sm hover:scale-105 transition-all shadow-lg"
                    >
                      <PhoneCall size={18} /> Appeler (+237)
                    </a>
                    <button 
                      onClick={() => {
                        const text = `Service de Livraison Douala MJ Logistics : 1500 F pour les résidents, 2000 F pour l'international. Stockage disponible. Contact : +237 699932926. ${window.location.origin}`;
                        window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
                      }}
                      className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-emerald-500 text-emerald-600 rounded-xl font-bold text-sm hover:scale-105 transition-all shadow-lg"
                    >
                      <Send size={18} /> Partager l'offre
                    </button>
                  </div>
                </div>
                <div className="w-full lg:w-1/3">
                  <div className="relative">
                    <img 
                      src="https://picsum.photos/seed/delivery/600/800" 
                      alt="Douala Logistics" 
                      className="rounded-3xl shadow-2xl border-4 border-white"
                      referrerPolicy="no-referrer"
                    />
                    <div className="absolute -bottom-6 -left-6 p-6 bg-white rounded-3xl shadow-xl border border-slate-100 max-w-[200px]">
                      <div className="flex items-center gap-2 text-emerald-500 mb-2">
                        <CheckCircle size={16} />
                        <span className="text-[10px] font-black uppercase">Fiabilité MJ</span>
                      </div>
                      <p className="text-[10px] text-slate-500 leading-tight">
                        Gestion des flux de ventes et rapports de stock en temps réel.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {/* Financial Projections & Visibility Section */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 data-card">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-black flex items-center gap-2">
                    <LineChartIcon className="text-emerald-500" /> Projections Financières (Bénéfices Nets)
                  </h3>
                  <div className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-bold">ESTIMATIONS 2026</div>
                </div>
                
                <div className="space-y-6">
                  {[
                    { period: '1 Mois', min: 500, max: 2500, growth: '+15%', color: 'bg-blue-500' },
                    { period: '6 Mois', min: 5000, max: 25000, growth: '+120%', color: 'bg-indigo-500' },
                    { period: '1 An', min: 20000, max: 150000, growth: '+500%', color: 'bg-emerald-500' },
                  ].map((proj) => (proj && (
                    <div key={proj.period} className="relative">
                      <div className="flex justify-between items-end mb-2">
                        <div>
                          <div className="text-xs font-bold text-slate-400 uppercase">{proj.period}</div>
                          <div className="text-lg font-black text-slate-900">
                            {formatPrice(proj.min)} — {formatPrice(proj.max)}
                          </div>
                        </div>
                        <div className="text-xs font-bold text-emerald-500">{proj.growth}</div>
                      </div>
                      <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden flex">
                        <div className={`h-full ${proj.color} opacity-30`} style={{ width: '30%' }} />
                        <div className={`h-full ${proj.color}`} style={{ width: '40%' }} />
                      </div>
                      <div className="flex justify-between mt-1 text-[8px] font-bold text-slate-400 uppercase">
                        <span>Minima</span>
                        <span>Cible Optimale</span>
                        <span>Potentiel Max</span>
                      </div>
                    </div>
                  )))}
                </div>
                <p className="mt-6 text-[10px] text-slate-400 italic">
                  * Ces projections sont basées sur un taux de commission moyen de 12% et une croissance mensuelle de l'écosystème de 20%.
                </p>
              </div>

              <div className="data-card bg-slate-900 text-white border-none">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <Eye className="text-emerald-400" /> Visibilité en Ligne
                </h3>
                <div className="space-y-4">
                  {[
                    { name: 'Marketplace B2B/B2C', level: 'Haute', color: 'text-emerald-400', desc: 'Indexé Google, Public' },
                    { name: 'MJ Logistics Douala', level: 'Moyenne', color: 'text-blue-400', desc: 'Ciblage E-commerçants' },
                    { name: 'Trading & Betting', level: 'Haute', color: 'text-emerald-400', desc: 'Trafic Viral, Temps Réel' },
                    { name: 'AI Services (Veo)', level: 'Moyenne', color: 'text-amber-400', desc: 'Accès Premium/API' },
                    { name: 'Academy', level: 'Basse', color: 'text-slate-400', desc: 'En cours de déploiement' },
                  ].map((service) => (
                    <div key={service.name} className="p-3 bg-white/5 rounded-xl border border-white/10">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-bold">{service.name}</span>
                        <span className={`text-[10px] font-black uppercase ${service.color}`}>{service.level}</span>
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Referral Program */}
              <div className="data-card bg-gradient-to-br from-indigo-600 to-violet-700 text-white border-none overflow-hidden relative">
                <div className="absolute top-0 right-0 p-8 opacity-10">
                  <Users size={120} />
                </div>
                <div className="relative z-10">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/20 rounded-full text-[10px] font-black uppercase tracking-widest mb-4">
                    <Sparkles size={12} /> Programme de Parrainage
                  </div>
                  <h3 className="text-2xl font-black mb-2">Gagnez en Recommandant</h3>
                  <p className="text-sm text-indigo-100 mb-6">
                    Partagez MJ NEXUS avec votre réseau. Recevez 5% de commission sur chaque transaction de vos filleuls.
                  </p>
                  <div className="p-4 bg-white/10 rounded-2xl border border-white/20 flex items-center justify-between gap-4">
                    <div className="text-xs font-mono truncate opacity-80">mjnexus.com/ref/user_8291</div>
                    <button className="px-4 py-2 bg-white text-indigo-600 rounded-xl font-bold text-xs hover:bg-indigo-50 transition-all">
                      Copier
                    </button>
                  </div>
                </div>
              ...
              <div className="data-card border-slate-200">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold flex items-center gap-2">
                    <Heart className="text-rose-500" /> Votre Satisfaction
                  </h3>
                ...
                <p className="text-sm text-slate-500 mb-6">
                  MJ NEXUS s'engage à offrir une expérience d'exception. Comment pouvons-nous nous améliorer ?
                </p>
                <div className="space-y-3">
                  <textarea 
                    placeholder="Votre avis nous intéresse..."
                    className="w-full h-24 bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:border-emerald-500 resize-none"
                  />
                  <button 
                    onClick={() => {
                      alert('Merci pour votre feedback ! Nous l\'analyserons pour améliorer nos services.');
                    }}
      ...
      case 'servisecur':
        return (
                ...
                <div className="data-card">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                      ...
                      <button 
                        key={i} 
                        onClick={() => initiateBooking(cat.name)}
                        className="flex flex-col items-center gap-3 p-4 rounded-2xl border border-slate-100 hover:border-emerald-500 transition-all group"
                      >
          ...
          <Suspense fallback={<div className="p-12 text-center text-slate-400 animate-pulse">Chargement de la carte...</div>}>
        ...
        return (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* WhatsApp Contact for Payments */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-xl text-center">
                <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6">
                  <MessageSquare size={40} />
                </div>
                <h3 className="text-2xl font-black text-slate-900 mb-4">Paiements & Recharge</h3>
                <p className="text-slate-500 mb-8 max-w-md mx-auto">
                  Pour toute recharge de compte, retrait ou paiement sécurisé, veuillez contacter directement notre service client sur WhatsApp.
                </p>
                <button 
                  onClick={() => window.open('https://wa.me/237699932926', '_blank')}
                  className="px-10 py-5 bg-emerald-500 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-2xl shadow-emerald-500/40 flex items-center justify-center gap-3 mx-auto"
                >
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {/* Global Trust Banner */}
        <div className="bg-slate-900 text-white py-1 px-8 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest z-50">
          <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1 text-blue-400">
              <Truck size={10} />
              Douala Logistics: +237 699932926
            </span>
            ...
            >
              <div className="mb-8">
                <h1 className="text-3xl font-black text-slate-900 capitalize italic">
                  {activeTab === 'home' ? 'MJ NEXUS : Votre Partenaire Business' : activeTab === 'dashboard' ? 'Welcome back, Joël' : activeTab.replace('-', ' ')}
                </h1>
                <p className="text-slate-500">
                  {activeTab === 'home' ? 'L\'écosystème unifié n°1 au Cameroun.' : '"Gagnez avec le monde. Pariez autrement."'}
                </p>
              </div>
              {renderContent()}
            </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="p-8 space-y-4">
                  ...
                  {authMode === 'register' && (
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Account Type</label>
                      <div className="grid grid-cols-2 gap-2">
                        <button 
                          onClick={() => setAuthForm({...authForm, role: 'client'})}
                          className={`py-2 rounded-xl text-xs font-bold border transition-all ${authForm.role === 'client' ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
                        >
                          Client
                        </button>
                        <button 
                          onClick={() => setAuthForm({...authForm, role: 'admin'})}
                          className={`py-2 rounded-xl text-xs font-bold border transition-all ${authForm.role === 'admin' ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
                        >
                          Administrator
                        </button>
                      </div>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Phone or Email</label>
                    <input 
                      type="text" 
                      ...
                      placeholder="e.g. +237 6XX XXX XXX or email@example.com" 
                    />
                  </div>
                  <div>
                  ...
                  <button 
                    ...
                    className={`w-full py-4 bg-emerald-500 text-white rounded-2xl font-bold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 ${isAuthLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    {isAuthLoading ? (
                      <>
                        <RefreshCw size={18} className="animate-spin" />
            ...
            >
              ...
              >
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">Commander une Intervention</h3>
                    <div className="flex items-center gap-1 text-[10px] text-emerald-600 font-bold uppercase mt-0.5">
                      <Shield size={12} />
                      Service Sécurisé & Traçable
                    </div>
                  </div>
                  <button onClick={() => setShowBookingModal(false)} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-all">
                    <X size={20} />
                  </button>
                </div>
                <div className="p-6 space-y-4">
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Type de Service</label>
                    <select 
                    ...
                    >
                      ...
                      <option value="Drone Pro">Surveillance par Drone</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Description du Problème</label>
                    <textarea 
